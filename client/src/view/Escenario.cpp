...
void Escenario::draw(){
	...
	if (!viga1.loadFromFile("vigah.png"))
		return;
	viga1.render(100,210); //se renderea pasandole la posicion x, y el angulo en sentido horario(opcional)

	LTexture viga2(window);
	if (!viga2.loadFromFile("vigah.png"))
		return;
	viga2.render(235,222, 10);

	LTexture viga3(window);
	if (!viga3.loadFromFile("vigah.png"))
		return;
	viga3.render(450,222, -10); //se renderea pasandole la posicion x, y el angulo en sentido horario(opcional)

	LTexture viga4(window);
	if (!viga4.loadFromFile("vigah.png"))
		return;
	viga4.render(585,210);
